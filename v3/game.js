...
serverUrl+=":"+port;}
this.socket=io(serverUrl,{withCredentials:false});if(this.socket){connecting=true;}}else{if(this.socket.disconnected){this.socket.connect();connecting=true;}}
...
this.countdownTime=8.6;this.timer=new ig.Timer(this.countdownTime);this.timer.pause();this.countdowning=false;this.radius=this.avatar.width/2;this.centerX=this.avatarX+this.radius;this.centerY=this.avatarY+this.radius;this.PI2=Math.PI*2;this.rAngle=0;},showEmoji:function(eID){ig.game.spawnEntity(EntityEmojiAnim,this.avatarX+this.avatar.width/2,this.avatarY+this.avatar.height/2,{eID:eID});},pause:function(){if(this.countdowning)this.timer.pause();},unpause:function(){if(this.countdowning)this.timer.unpause();},clicked:function(){var temp=this.clickTime;this.clickTime=ig.system.clock.delta();if(this.clickTime-temp<0.35){return;}
if(ig.game.currentLayer!=ig.game.layers.BASE)return;if(!this.parents.transferingTurn){this.autoMode=!this.autoMode;if(this.autoMode){this.autoPlay();}else if(this.timeOut!=null){clearTimeout(this.timeOut);this.timeOut=null;}}},autoPlay:function(){if(this.parents.gameEnd)return;if(this.timeOut==null){this.timeOut=setTimeout(function(){if(this==this.parents.currentPlayer&&!this.parents.dice.isRolling){if(this.parents.dice.isReady){this.parents.dice.rollingBot();}else{if(this.checkHorseJumping())this.checkSelecting();}}
...
console.log('Full player data:',this.players);if(ig.game.gameInitData){for(var i=0;i<this.players.length;i++){if(this.players[i].isMainPlayer&&this.players[i].rankNo===1){console.log('🏆 Winner detected! Sending score for rank 1');if(typeof sendScore==='function'){sendScore(1);}
break;}}}
